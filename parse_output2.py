...
# script to parse output of plot_sdm_err_vs_nrows and plot values
...
pat3=re.compile(r'[^\n]*\nperr=(\d+), starting nrows=(\d+), nact=(\d), epochs=(\d+),\s*\n'
	'bsm_err=([^,]+), emp_err=([^,]+), std=([^\n]+)\n')
...
for (perr, nrows, nact, epochs, bsm_er, emp_err, std) in re.findall(pat3, output):
	print("found nrows=%s" % nrows)
	perr = int(perr)
	nrows = int(nrows)
	nact = int(nact)
	epochs = int(epochs)
	...
	rows.append(nrows)
	nacts.append(nact)
	emp_mean.append(emp_err)
	emp_clm.append((std / math.sqrt(epochs)) * 1.96)
	bsm_err.append(bsm_er)
...
xvals = rows
xaxis_labels = ["%s/%s" % (rows[i], nacts[i]) for i in range(len(rows))]
for pi in plots_info:
	plt.subplot(pi["subplot"])
	plt.errorbar(xvals, emp_mean, yerr=emp_clm, fmt="-o", label="empirical")
	plt.errorbar(xvals, bsm_err, yerr=None, fmt="-o", label="bsm predicted")
	title = "SDM error rate vs nrows for binarized sdm"
	if pi["scale"] == "log":
		plt.yscale('log')
		title += " (log scale)"
