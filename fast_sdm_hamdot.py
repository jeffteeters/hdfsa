...
class Fast_sdm_empirical():
	...
	def __init__(self, nrows, ncols, nact, actions=10, states=100, choices=10, epochs=10,
			...
			hl_selection_method="hamming", bits_per_counter=8, match_method="both", only_one_distractor = False,
		...
		# match_method: "hamming" to threshold sum to binary number before comparing to item memory (hamming distance)
		#  this is the normal SDM.  "dot" match to item memory done via dot product; "both" do both (to compare)
		...
		threshold_sum = match_method in ("hamming", "both")
		use_dot_product = match_method in ("dot", "both")
		# self.empiricalError()
	...
	# def empiricalError(self):
		...
		fail_counts_dot = np.zeros(self.epochs, dtype=np.uint16)
		if threshold_sum:
		...
		if use_dot_product:
			...
			distance_counts_len_dot = 5*(self.ncols * self.nact)  # should be enough range
			distance_counts_offset_dot = int(distance_counts_len_dot / 2)
			self.match_hamming_counts_dot = np.zeros(distance_counts_len_dot, dtype=np.uint32)
			self.distractor_hamming_counts_dot = np.zeros(distance_counts_len_dot, dtype=np.uint32)
		...
		for epoch_id in range(self.epochs):
			...
			if use_dot_product:
				# if using dot product, make version of counters with threshold
				contents_dot = contents.copy()
				contents_dot[contents_dot > 1] = 1
				contents_dot[contents_dot < -1] = -1
				# replace zero counter values with random +1 or -1
				random_plus_or_minus_one = rng.integers(0, high=2, size=(self.nrows, self.ncols), dtype=np.int8)*2-1
				# random_plus_or_minus_one = np.random.randint(0, high=2, size=(self.nrows, self.ncols), dtype=np.int8)*2-1
				mask = contents_dot == 0
				contents_dot[mask] = random_plus_or_minus_one[mask]

				# Use +1/-1 product and dot product with item memory to calculate distance
				address_dot = address*2-1    # convert address to +1/-1
				im_state_dot = im_state*2-1  # convert im_state to +1/-1
			for i in range(num_transitions):
				...
				if threshold_sum:
					...
					recalled_data = np.roll(np.logical_xor(address[i], recalled_vector), -1).astype(np.int8)
				...
				if use_dot_product:
					...
					# first, truncate counters to binary
					recalled_vector = contents_dot[transition_hard_locations[i,:]].sum(axis=0)
					recalled_data_dot = np.roll(address_dot[i] * recalled_vector, -1)
					dot_distances = np.sum(im_state_dot[:,] * recalled_data_dot, axis=1) # actually dot product distance
					# normal processing, are multiple distractors
					two_smallest = np.argpartition(dot_distances, 2)[0:2]
					if dot_distances[two_smallest[0]] < dot_distances[two_smallest[1]]:
						if transition_next_state[i] != two_smallest[0]:
							fail_counts_dot[epoch_id] += 1
							fail_count_dot += 1
							dot_failed = True
					elif dot_distances[two_smallest[1]] < dot_distances[two_smallest[0]]:
						if transition_next_state[i] != two_smallest[1]:
							fail_counts_dot[epoch_id] += 1
							fail_count_dot += 1
							dot_failed = True
				...
				trial_count += 1
				# show for debugging
				if ham_failed != dot_failed:
					ham_dot_mismatch += 1
					# print("Found ham_dot_mismatch")
					# print("contents[hl]    =%s" % contents[transition_hard_locations[i,:]])
					# print("contents_dot[hl]=%s" % contents_dot[transition_hard_locations[i,:]])
					# print("recalled_data    =%s" % recalled_data)
					# print("recalled_data_dot=%s" % recalled_data_dot)
					# print("hamming_distnaces=%s" % hamming_distances)
					# print("dot_distances    =%s" % dot_distances)
					# print("hard_locations=%s" % transition_hard_locations[i,:])
					# import pdb; pdb.set_trace()
					# print("ham_failed=%s, dot_failed=%s" % (ham_failed, dot_failed))
		...
		self.fail_count_dot = fail_count_dot
		...
		print("fail_count=%s / %s (%s%%); fail_cound_dot=%s/%s (%s%%)" % (
			fail_count, trial_count, fail_count*100/trial_count, fail_count_dot, trial_count,
			fail_count_dot*100/trial_count))
		print("ham_dot_mismatch = %s/%s (%s%%)" % (ham_dot_mismatch, trial_count,
			round(ham_dot_mismatch*100/ trial_count,3)))
		# print("fast_sdm_empirical epochs=%s, perr=%s, std_error=%s" % (self.epochs, perr, self.std_error))


def main():
	# ncols = 16; actions=3; choices=3; states=3
...
#
	...
	bits_per_counter = 8
	# nrows = 51; nact=1  # 10^-1 error
	# nrows = 86; nact=2  # 10^-2 error
	# nrows = 125; nact=2 # 10^-3 error
	nrows = 168; nact=2 # 10^-4 error
	# nrows=6; nact=1; bits_per_counter=8; only_one_distractor = False
	epochs=1000
	fse = Fast_sdm_empirical(nrows, ncols, nact, actions=actions, states=states, choices=choices,
		bits_per_counter=bits_per_counter, only_one_distractor=only_one_distractor,
		epochs=epochs)
	print("With nrows=%s, ncols=%s, nact=%s, only_one_distractor=%s,"
		" epochs=%s, mean_error=%s, std_error=%s" % (nrows, ncols,
		nact, only_one_distractor, epochs, fse.mean_error, fse.std_error))
