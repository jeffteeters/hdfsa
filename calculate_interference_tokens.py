...
# fsa is:
# s0 + a0 -> s1  (locked + add token -> unlocked)
# s0 + a1 -> s0  (locked + push -> locked)
# s1 + a0 -> s1  (unlocked + add token -> unlocked)
# s1 + a1 -> s0  (unlocked + push -> locked)


def calc_result(bun, s0, s1, a0, a1, p0, p1):
	read1 = "match" if s0^a0^bun == p1 else "fail"
	read2 = "match" if s0^a1^bun == p0 else "fail"
	read3 = "match" if s1^a0^bun == p1 else "fail"
	read4 = "match" if s1^a1^bun == p0 else "fail"
	result = "%s %s %s %s" % (read1, read2, read3, read4)
	fail_count = result.count("fail")
	return (result, fail_count)
	return result

def calc_result_orig(bun, s0, s2, a0, a2, p0, p1):
...
for opt in possible_options:
	...
	sidx += 1
	if bundle_sum == 0:
		(result_0, fail_count_0) = calc_result(0, s0, s1, a0, a1, p0, p1)
		(result_1, fail_count_1) = calc_result(1, s0, s1, a0, a1, p0, p1)
		...
		(result, fail_count) = calc_result(bun, s0, s1, a0, a1, p0, p1)
		error_count += fail_count
	print("s0-%s, s1-%s, a0-%s, a1-%s, p0-%s, p1-%s; result = %s" % (s0, s1, a0, a1, p0, p1, result))
